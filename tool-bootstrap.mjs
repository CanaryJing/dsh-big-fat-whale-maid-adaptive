 ...
 * hand the promoted catalog to router-progressive (four-phase progressive
 * disclosure). Injected-context control lives in the companion
 * `context-gate` plugin, not here.
 *
 * Adapted from dsh-anchored-standard/preset/tool-bootstrap.mjs (remote HEAD):
 *   - PROMOTED phase returns the FULL catalog instead of the resident set —
 *     router-progressive owns post-promotion tool disclosure (phases +
 *     dev_tool_search unlock), so this filter must not narrow it again.
 *   - Everything else is unchanged: epoch-aware promotion (compaction/end
 *     demotes to the bootstrap pair + compactionTools), includeSubagents,
 *     optional bootstrapMaxTokens, fail-open robustness.
 *
...
export function apply(ctx, config) {
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    try {
      ...
      if (status.promoted) {
        // PROMOTED: return the FULL catalog — router-progressive owns the
        // post-promotion progressive disclosure (four phases + dev_tool_search
        // unlock), so this filter must not narrow it again.
        return assembled
      }
