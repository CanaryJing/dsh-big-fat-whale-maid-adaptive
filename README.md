...
> DSH（DeepSeek Harness）自定义 Agent Preset 插件：首轮以 Linux 极简双工具「开智」锚定，自动探测宿主环境（原生 Linux / 原生 Windows / WSL 内外）与工作目录所属世界，`bash` 直达 WSL 发行版、`gitbash` 默认操作 Windows 母系统（`pwsh` 兜底），晋升后开放全量工具，全程保持蓝发蓝瞳鲸鱼娘女仆长人设。
>
...
| 4 | 双世界 shell 路由 | Windows 宿主上：`bash` 工具**直达 WSL 发行版里的 bash**（预设自带的 wsl-bash 实现，通过 `wsl.exe` 调用），`gitbash` 工具默认操作 Windows 母系统（文件操作更顺手，`pwsh` 在 Git Bash 不可用或需要 PowerShell 专属能力时兜底）；Linux 宿主上全原生，检测到 pwsh 才注册；WSL 内运行时可通过 PowerShell interop 触达 Windows 母系统 |
...
- ✅ **win32 双 shell 路由**：Windows 宿主上默认注册 `gitbash`（Git Bash）操作 Windows 母系统文件；Git Bash 不可用或需要 PowerShell 专属能力（注册表/服务/WMI 等）时自动回退 `pwsh`（优先 PowerShell 7 的 `pwsh.exe`，兜底 Windows PowerShell 5.1 的 `powershell.exe`）。
...
├── env-probe.mjs         # 环境探测 + 简报注入 + env_probe 工具
